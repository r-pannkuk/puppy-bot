...
export class GuildMessageScanner implements IGuildManager {
	...
	public async fetchAllMessages(lastMessageStore: LastMessageStore) {
		...
		const counts = await Promise.all(channels.map((channel) => this.fetchAllMessagesInChannel({
			channel,
	...
	public async fetchAllMessagesInChannel(args: {
		...
		async function recursiveSearch(lastMessageId?: string) {
			const MAX_FETCH = 100;
			...
			try {
				...
				return recursiveSearch(lastMessageId);
			}
			...
			if (messages.size < MAX_FETCH) {
				...
				return recursiveSearch(lastMessage!.id);
			}
		}
